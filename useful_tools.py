...
# input d is the dimension
def generate_uniform_points_in_annulus(d,r,R,N):
    points = numpy.random.normal(size=(N,d))
    scales = (numpy.random.uniform(r,R,(N,)))**(1/d)
    for i in range(N):
        points[i,:] = points[i,:]/numpy.sqrt(numpy.sum(points[i,:]**2))*scales[i]
    return points
...
# generate uniform distributed points on the boundary of domain {|x|<R}
def generate_uniform_points_on_annulus(d,r,R,N_boundary):
    if d == 1:
...
def generate_learning_rates(highest_lr_pow,lowest_lr_pow,total_iterations):
    lr = 10 ** (highest_lr_pow + (lowest_lr_pow-highest_lr_pow)/total_iterations*numpy.arange(0,total_iterations+1))
    return lr
...
#pts = generate_uniform_points_in_annulus(2,0.4,1,1000)
#matplotlib.pyplot.plot(pts[:,0],pts[:,1],'.r')
#matplotlib.pyplot.show()
